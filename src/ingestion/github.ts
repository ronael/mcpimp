...
export class GitHubSourceAdapter implements ContentSourceAdapter<GitHubSourceDefinition> {
   ...
   *
   * `SKILL.md` drives discovery for this GitHub skill adapter. Component
   * detection stays scoped to the discovered capability roots: if a `mcp.json`
   * belongs to the same root it is announced as an `mcp` component, but MCPIM
   * never scans globally for `mcp.json` outside those roots.
   */
  async discover(source: GitHubSourceDefinition, revision: SourceRevision): Promise<DiscoveredCapability[]> {
    ...
    for (const root of skillRoots) {
      ...
      // Skill-specific metadata, classified from paths alone: discovery must stay
      // download-free so an unchanged capability costs nothing. The manifest
      // refines this with the SKILL.md text once the content is actually fetched.
